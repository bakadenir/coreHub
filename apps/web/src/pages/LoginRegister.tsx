...
export default function LoginRegister() {
    ...
    const handleRegister = async (e: React.FormEvent) => {
        ...
        try {
            if (!registerData.username) throw new Error('Username required');
    ...
    const socialButtons = (
        <div className="flex gap-4 mb-4">
            {/* Google */}
            <button type="button" className="w-10 h-10 rounded-full border border-gray-300 flex items-center justify-center hover:bg-gray-50 transition-colors bg-white shadow-sm" onClick={() => showToast('Google login coming soon!', 'info')}>
            ...
            {/* GitHub */}
            <button type="button" className="w-10 h-10 rounded-full border border-gray-300 flex items-center justify-center hover:bg-gray-50 transition-colors bg-white shadow-sm" onClick={() => showToast('GitHub login coming soon!', 'info')}>
            ...
            {/* Facebook */}
            <button type="button" className="w-10 h-10 rounded-full border border-gray-300 flex items-center justify-center hover:bg-gray-50 transition-colors bg-white shadow-sm" onClick={() => showToast('Facebook login coming soon!', 'info')}>
    ...
    return (
        <div className="bg-background-light text-gray-900 font-sans antialiased min-h-screen flex flex-col transition-colors duration-300 relative overflow-hidden">
            ...
            <main className="flex-1 flex items-center justify-center p-4 relative z-10 animate-fade-in-up">
                {/* Main Container */}
                <div className={`relative bg-white rounded-xl shadow-lg shadow-gray-200/50 overflow-hidden w-full max-w-[850px] min-h-[600px] transition-all duration-300 border border-gray-200`}>

                    {/* Sign Up Form Container */}
                    <div className={`absolute top-0 h-full w-1/2 transition-all duration-700 ease-in-out left-0 flex flex-col items-center justify-center p-12 bg-white
                        ...
                        <form onSubmit={handleRegister} className="w-full flex flex-col items-center" noValidate>
                            ...
                            <button
                                type="submit"
                                disabled={isLoading}
                                className="mt-8 flex w-full justify-center items-center gap-2 rounded-xl bg-black px-3 py-2.5 text-sm font-bold text-white shadow-md hover:bg-gray-800 disabled:opacity-70 disabled:cursor-not-allowed transition-all"
                            >
                                {isLoading ? (
                                    <>
                    ...
                    <div className={`absolute top-0 h-full w-1/2 transition-all duration-700 ease-in-out left-0 z-20 flex flex-col items-center justify-center p-12 bg-white
                        ...
                        <form onSubmit={handleLogin} className="w-full flex flex-col items-center" noValidate>
                            ...
                            {errors.login && (
                                <p className="flex items-center gap-2 text-sm text-red-600 font-medium animate-fade-in pl-1 mb-4 w-full text-left justify-start">
                                    <span className="material-icons-outlined text-[16px]">error</span>
                                    {errors.login}
                                </p>
                            )}
                    ...
                    <div className={`absolute top-0 left-1/2 w-1/2 h-full overflow-hidden transition-transform duration-700 ease-[cubic-bezier(0.25,0.1,0.25,1.0)] z-[100] 
                        bg-black text-white outline outline-1 outline-white/10
                        ...
                        <div className={`absolute inset-0 flex flex-col items-center justify-center px-16 text-center transition-all duration-700 ease-in-out
                            ...
                            <button
                                className="rounded-xl border-2 border-white/20 bg-white/5 backdrop-blur-sm px-8 py-2.5 text-sm font-bold text-white uppercase tracking-wider hover:bg-white hover:text-black transition-all duration-300 shadow-lg"
